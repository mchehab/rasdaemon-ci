...
for script in ghes_inject.py arm_processor_error.py ghes_decode.py \
	qmp_helper.py pci_bus_error.py fuzzy_error.py raw_error.py; do
	cp "$work/source/scripts/$script" \
		"$output/root/opt/qemu/libexec/rasdaemon/$script"
done
qmp_wheel=$(find "$work/source/python/wheels" -maxdepth 1 \
	-name 'qemu_qmp-*.whl' -print -quit)
test -n "$qmp_wheel"
cp "$qmp_wheel" "$output/root/opt/qemu/libexec/rasdaemon/"
